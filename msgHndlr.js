...
const { compor } = require('./lib/jogos')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const ownerNumber = ["5599984397286@c.us", "55xxxxx"] // replace with your whatsapp number
        ...
        switch (command) {
            ...
            case '!sgif':
                if (isMedia) {
                    if (mimetype === 'video/mp4' && message.duration < 15 || mimetype === 'image/gif' && message.duration < 15) {
                        const mediaData = await decryptMedia(message, uaOverride)
                        client.reply(from, 'Espera ae só um tiquim, fiote', id)
            ...
            case '!stikernobg':
                if (isMedia) {
                    client.sendText(from, `Talvez não funcione, viu... e é isso!!`)
                    try {
            ...
            case '!compor':
                ...
                var paraCompor = args.join(" ").replace(args[0] + " ", "");
                ...
                client.sendText(from, textAux)
                break
            case '!repita':
                cliente.sendText(from, args.join(" ").replace(args[0] + " ", ""));
                break
            case '!tts':
